...
The implementation is solid overall, with strong test coverage across all layers. Three actionable findings require correction before the module can be considered production-ready.
...
**Fix**: Read `INFRAMAP_MASTER_KEY` from environment, create `crypto.AESGCMEncryptor` when the key is present, and pass it to `NewPgDiscoveryRepository`. Log a warning when the key is absent (dev/test mode).
