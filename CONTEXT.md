...
49. **CodeRabbit Review Thread Resolution in Merge Automation**: When automation or PR monitoring loops merge a PR via GitHub CLI or GraphQL, unresolved review threads block the `mergePullRequest` mutation. The agent MUST inspect review threads via GraphQL (`reviewThreads`), implement code fixes, and explicitly resolve each thread via `resolveReviewThread` before finalizing the merge.
